...
def get_or_create_split_ids(subjects, split_path="split_ids.json", ratio=0.8, seed=42):
    """
    Save and reuse a fixed split of patient IDs for reproducibility.

    Args:
        subjects (list): List of dictionaries with at least a 'patientID' key.
        split_path (str): File where splits are stored.
        ratio (float): Ratio for training set.
        seed (int): Seed for reproducibility.

    Returns:
        train_ids, val_ids (list of str): Lists of patient IDs.
    """
    ...
    print(f"No existing split file found, generating new one and saving to {split_path}")
    all_ids = [s["patientID"] for s in subjects]
    random.seed(seed)
    random.shuffle(all_ids)
    split_index = int(len(all_ids) * ratio)
...
def get_subjects_2phases(path):
    ...
    for sbj_file in sbj_files:
        ...
        #subject_data_phase_1 = torch.permute(subject_data_phase_1, [3, 1, 2, 0])
        #subject_data_phase_3 = torch.permute(subject_data_phase_3, [3, 1, 2, 0])

        subject_data_phase_1 = subject_data_phase_1.permute(3, 1, 2, 0)  # [D, H, W, 1]
        subject_data_phase_3 = subject_data_phase_3.permute(3, 1, 2, 0)  # [D, H, W, 1]
        print(f"loaded shape: {subject_data_phase_3.shape}")
...
class WholeImageDataset:
    def __init__(self, path, ratio=0.8, batch_size=1, patch_size=(128, 128, 32),
                 samples_per_volume=32, max_length=200, num_workers=8, split_file="split_ids.json"):
        ...
        # Load or create split using patient IDs
        train_ids, val_ids = get_or_create_split_ids(self.subjects, split_path=split_file, ratio=ratio)

        # Filter based on split
        self.training_subjects = [s for s in self.subjects if s["patientID"] in train_ids]
        self.validation_subjects = [s for s in self.subjects if s["patientID"] in val_ids]

        # Optional sanity check
        print(f"Training: {len(self.training_subjects)} subjects")
        print(f"Validation: {len(self.validation_subjects)} subjects")
    ...
    def get_loaders(self):
        train_loader_patches = torch.utils.data.DataLoader(
            self.patches_training_set, batch_size=self.batch_size, drop_last=True, num_workers=0
        )
        val_loader_patches = torch.utils.data.DataLoader(
            self.patches_validation_set, batch_size=1, drop_last=True, num_workers=0
        )

        return train_loader_patches, val_loader_patches


class WholeImageDataset2:
    def __init__(self, path, ratio=0.8, batch_size=1, patch_size=(128, 128, 32), samples_per_volume=32, max_length=200,
                 num_workers=8):
        """
    ...
    def get_loaders(self):
        ...
        train_loader_patches = torch.utils.data.DataLoader(self.patches_training_set, batch_size=self.batch_size,
                                                           drop_last=True, num_workers=0)
        val_loader_patches = torch.utils.data.DataLoader(self.patches_validation_set, batch_size=1, drop_last=True,
                                                         num_workers=0)
